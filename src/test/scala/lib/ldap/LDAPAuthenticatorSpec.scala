...
class LDAPAuthenticatorSpec extends FunSpec with Matchers  {
  ...
  private def mkSetting(bindPassword:String):LDAPSetting = {
    LDAPSetting("localhost", 389, Plain, "dc=example,dc=com", "cn=admin,ou=Users,dc=example,dc=com", bindPassword, "uid", "mail")
  }
  ...
  it("success bindDN and userDN") {withRule(embeddedLdapRule()) { _ =>
  ...
  it("success bindDN, but user auth is fail by invalid password"){withRule(embeddedLdapRule()) { _ =>
  ...
  it("success bindDN but user not found. "){withRule(embeddedLdapRule()) { _ =>
    val res = LDAPAuthenticator.authenticateUser(mkSetting("adminpass"), "user2", "user2pass")
    res.isLeft should equal(true)
    res.left.get should equal("user not found.")
  }}

  it("fail bindDN by invalid password"){withRule(embeddedLdapRule()) { _ =>
  ...
  it ("fail bindDN by invalid dn") {withRule(embeddedLdapRule()) { _ =>
  ...
  it("server connection fail by invalid setting") {withRule(embeddedLdapRule()) { _ =>
